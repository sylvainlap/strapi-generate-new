...
module.exports = function afterGenerate(scope, cb) {
  ...
  async.auto({
    ...
    userDependencies: function (cb) {
      fs.mkdir(path.resolve(scope.rootPath, 'node_modules'), cb);
      const userPkg = JSON.parse(fs.readFileSync(path.resolve(scope.strapiRoot, 'node_modules', 'strapi-generate-users', 'package.json')));
      _.forEach(userPkg.dependencies, function (version, dependency) {
        const srcDependencyPath = path.resolve(scope.strapiRoot, 'node_modules', 'strapi-generate-users', 'node_modules', dependency);
